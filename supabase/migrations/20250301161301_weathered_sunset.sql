...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'auth_logs' AND rowsecurity = true
  ) THEN
...
-- Create policy for inserting auth logs (allow all authenticated users and anonymous users)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'security_settings' AND rowsecurity = true
  ) THEN
...
DROP POLICY IF EXISTS "Users can create their own security settings" ON security_settings;
...
DROP TRIGGER IF EXISTS update_session_last_active ON user_sessions;
