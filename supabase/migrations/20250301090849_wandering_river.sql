/*
  # Certificate status and storage access

  1. Functions
    - `increment_link_view_count()` - Increments view count when access is logged
    - `update_certificate_status()` - Updates certificate status based on expiry date
  
  2. Triggers
    - `on_access_log_insert` - Triggers view count increment
    - `before_certificate_insert_update` - Updates certificate status
  
  3. Security
    - Create storage policy for anonymous access to shared certificates
*/
