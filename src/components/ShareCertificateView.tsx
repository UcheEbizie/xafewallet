...
const ShareCertificateView = () => {
  ...
  const logAccess = async (shareId: string, certs: any[]) => {
    try {
      ...
      for (const cert of certs) {
        try {
          // Create a unique ID for the access log to use with upsert
          const accessLogId = `${cert.id}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
          
          // Use upsert instead of insert to avoid RLS policy issues
          await supabase
            .from('access_logs')
            .upsert({
              id: accessLogId,
  ...
  const handleDownload = async (certificate: any) => {
    try {
      ...
      try {
        // Create a unique ID for the access log to use with upsert
        const accessLogId = `${certificate.id}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
