...
// Add global type declaration for certificate files
declare global {
  interface Window {
    certificateFiles: Record<string, File>;
  }
}
...
const mockData = {
  ...
  settings: {
    ...
    upsert: async (settings: Partial<Settings>) => {
      return {
        ...settings,
        id: '1',
        user_id: '1',
  ...
  certificates: {
      ...
      return [
        {
          ...
          file_url: 'https://images.pexels.com/photos/5428836/pexels-photo-5428836.jpeg?auto=compress&cs=tinysrgb&w=800',
        ...
        },
        {
          ...
          file_url: 'https://images.pexels.com/photos/4439901/pexels-photo-4439901.jpeg?auto=compress&cs=tinysrgb&w=800',
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString()
        }
      ];
    ...
    getById: async (id: string) => {
      const certificates = await mockData.certificates.list();
      return certificates.find(cert => cert.id === id) || null;
    },
    create: async (certificate: Partial<Certificate>) => {
      return {
        ...certificate,
    ...
    update: async (id: string, certificate: Partial<Certificate>) => {
      return {
        ...certificate,
        id,
        updated_at: new Date().toISOString()
    ...
    delete: async () => {
      // Simulate deletion (no actual deletion in demo mode)
      return true;
    },
    uploadFile: async (file: File) => {
      console.log('Mock uploadFile called with file:', file.name, file.type, file.size);
      
      // Create a proper object URL for the file that will persist
      const objectUrl = URL.createObjectURL(file);
      console.log('Created object URL for uploaded file:', objectUrl);
      
      // Store the file reference in localStorage to persist across page refreshes
      try {
        // Store the file reference in window object to keep it accessible
        if (!window.certificateFiles) {
          window.certificateFiles = {};
        }
        
        // Generate a unique ID for this file
        const fileId = `cert_file_${Date.now()}`;
        window.certificateFiles[fileId] = file;
        
        // Return the object URL directly - this will work for the current session
        return objectUrl;
      } catch (err) {
        console.error('Error storing file reference:', err);
        return objectUrl;
      }
    }
  }
  ...
  settings: {
    ...
    async get() {
      try {
        ...
        if (error) {
          if (error.code === 'PGRST116') {
            // Settings doesn't exist, create default settings
  ...
  certificates: {
    ...
    async getById(id: string) {
      try {
        ...
        if (!user) {
          console.warn('No authenticated user, using demo mode');
          const cert = mockData.certificates.getById(id);
          console.log('Mock certificate retrieved:', cert);
          return cert;
        }
    ...
    async delete(id: string) {
      try {
        ...
        if (!user) {
          console.warn('No authenticated user, using demo mode');
          return mockData.certificates.delete();
        }
    ...
    async uploadFile(file: File) {
      try {
        ...
        if (!user) {
          console.warn('No authenticated user, using demo mode');
          console.log('Using mock uploadFile with file:', file.name, file.type, file.size);
          const url = await mockData.certificates.uploadFile(file);
          console.log('Mock upload returned URL:', url);
          return url;
        }
        ...
        console.log('Falling back to mock uploadFile with file:', file.name, file.type, file.size);
        const url = await mockData.certificates.uploadFile(file);
        console.log('Mock upload returned URL:', url);
        return url;
      }
